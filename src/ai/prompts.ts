...
## ⚠️ 출력 종료 규칙
위 "Takwell Intake | 최종 판단은 변호사 검토가 필요합니다" 줄에서 응답을 반드시 종료하세요.
다음과 같은 내용은 절대로 추가하지 마세요:
- "다음 단계", "추가 안내", "안내 사항" 등의 부가 섹션
- 의뢰인을 향한 마무리 인사나 부탁 (예: "초안을 같이 제출해주세요")
- 이모지로 끝나는 격려/안심 문구
브리핑 본문 형식 그대로만 출력하고, 그 뒤에는 어떤 글자도 붙이지 마세요.`;
